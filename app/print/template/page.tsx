...
export default async function PrintTemplatePage(props: { searchParams: Promise<{ bcode?: string }> }) {
  ...
  const { data: batch } = await supabase
    .from('batches')
    .select('*, wakalat_namas(serial_number, created_at)')
    .eq('batch_code', bcode)
    .single()

  if (!batch || !batch.wakalat_namas) return <div>Batch Not Found</div>

  const wnamas = batch.wakalat_namas.sort((a: any, b: any) => a.serial_number.localeCompare(b.serial_number))
  ...
  `}} />
      {wnamas.map((wnama: any, index: number) => {
        const isLast = index === wnamas.length - 1;
        const serial = wnama.serial_number;
        const microtext = `\u00A0${serial} - VALID ORIGINAL - `;

        // Format date: DD-MM-YYYY
        const dateObj = new Date(wnama.created_at);
        ...
        return (
          ...
          >
            ...
            <div className="absolute top-[8mm] left-[12mm] right-[12mm]"><Guilloche height={35} className="w-full opacity-60" /></div>
            <div className="absolute bottom-[8mm] left-[12mm] right-[12mm]"><Guilloche height={35} className="w-full opacity-60" /></div>
            ...
            <div className={`relative z-10 w-full h-full flex flex-col justify-between text-[16pt] ${urduFont.className}`} dir="rtl" >
              ...
              <div className="flex justify-between items-center     ">
                <div className="w-24 h-24 rounded-full border-2 border-black flex items-center justify-center bg-transparent shadow-sm ">
                  <img src='/logo.png' alt='logo' className=' logo-black' />
                </div>

                <div className="flex flex-col items-center justify-center  bg-transparent mr-[51px]">
              ...
              <div className="flex-1 flex flex-col relative mt-6  bg-transparent">
                ...
                <div className=" flex flex-col  px-4 bg-transparent relative z-10 ">
                  ...
                  <div className='flex-1 flex flex-col  bg-transparent relative z-10 px-[24px] h-fit'>
                    ...
                    <div className="text-[14pt] leading-[2.15] text-justify font-medium leading-[28px]">
                      ...
                      صاحب موصوف تا حد محنتانہ نقصانات یا ہرجانہ کے ذمہ دار ہوں گے۔ لیکن وکیل موصوف صدر مقام کچہری کے علاوہ اور جگہ سماعت ہونے یا
                      بروز تعطیل یا کچہری کے اوقات سے پیچھے ہونے پر مظہر کو کوئی نقصان پہنچے تو اس کے ذمہ دار اس کے واسطے کسی معاوضہ ادا کرنے یا محنتانہ
                      واپس کرنے کے بھی صاحب موصوف ذمہ دار نہ ہوں گے۔ مجھ کو کل ساختہ پرداختہ صاحب موصوف مثل کردہ ذات خود قبول ہوگا۔ صاحب
                      موصوف کو عرضی و جواب دعویٰ اور اجراء درخواست برآمدگی منسوخی ڈگری یکطرفہ درخواست حکم امتناعی یا قرقی یا گرفتاری قبل از فیصلہ اجراء
                      ڈگری بھی صاحب موصوف کو بشرط ادائیگی علیحدہ محنتانہ پیروی اختیار ہوگا۔ کہ مقدمہ مذکور یا اس کے کسی جزو کی کاروائی کے واسطے کسی دوسرے
                      ...
                      ہائے وغیرہ کا اختیار وکیل صاحب موصوف کوہوگا۔
                      <span className="text-center  font-bold ">
                        لہٰذا یہ وکالت نامہ لکھ دیا ہے کہ سند رہے۔ وکالت نامہ سن لیا ہے اور اچھی طرح سمجھ لیا ہے اور منظور ہے۔ مورخہ
                      </span>
                    </div>
                  </div>
